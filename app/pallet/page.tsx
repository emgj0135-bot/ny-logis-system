...
export default function PalletsPage() {
  ...
  const downloadExcel = async () => {
    try {
      ...
      alert("엑셀 파일 생성에 실패했어. 브라우저 콘솔(F12)을 확인해봐!");
    }
  };
            ...
            <tbody className="divide-y divide-slate-50 font-black">
              {currentItems.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50 transition-all">
                  ...
                  <td className="p-6 text-center"><span className={`inline-block px-3 py-1 rounded-lg text-[10px] font-black ${item.type === '출고' ? 'bg-red-50 text-red-500' : 'bg-blue-50 text-blue-500'}`}>{item.type}</span></td>
          ...
          <div className="bg-white w-full max-w-xl rounded-[2.5rem] p-10 animate-in zoom-in-95 shadow-2xl">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <textarea placeholder="KPP 전표번호" className="col-span-2 p-4 bg-slate-50 rounded-2xl font-bold text-xs outline-none shadow-inner resize-none" value={formData.kpp_number} onChange={e => setFormData({...formData, kpp_number: e.target.value})} />
              </div>
              <div className="grid grid-cols-3 gap-3">
                <div className="col-span-1 space-y-3">
                ...
                <textarea placeholder="AJ 전표번호" className="col-span-2 p-4 bg-slate-50 rounded-2xl font-bold text-xs outline-none shadow-inner resize-none" value={formData.aj_name} onChange={e => setFormData({...formData, aj_name: e.target.value})} />
              </div>
              <div className="flex gap-3 pt-4">
                <button onClick={handleSubmit} className="flex-1 bg-[#1a1c2e] text-white p-5 rounded-[1.5rem] font-black hover:bg-black">{isEdit ? '수정 완료하기' : '새 전표 등록하기'}</button>
