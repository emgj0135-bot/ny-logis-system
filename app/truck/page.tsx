...
export default function TruckPage() {
  ...
  const downloadExcel = () => {
    try {
      ...
      if (!XLSX) return alert("엑셀 라이브러리 로딩 중...");
      ...
      XLSX.writeFile(workbook, `용차배차현황_${new Date().toISOString().split('T')[0]}.xlsx`);
  ...
  const handleOrderSubmit = async () => {
    ...
    if (selectedOrder) {
      await supabase.from('truck_orders').update(submissionData).eq('id', selectedOrder.id);
      alert("수정 완료! ✨");
    } else {
    ...
    <div className="p-8 bg-slate-50 min-h-screen font-sans text-slate-800">
      ...
      <div className="bg-white p-7 rounded-[2.5rem] shadow-sm border border-slate-100 mb-8 space-y-6">
        <div className="flex flex-wrap gap-10">
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-3 font-bold">
              <input type="date" className="p-3 bg-slate-50 rounded-xl outline-none text-xs" value={filters.created_start} onChange={e => setFilters({...filters, created_start: e.target.value})} />
              <span className="text-slate-300">~</span>
              <input type="date" className="p-3 bg-slate-50 rounded-xl outline-none text-xs" value={filters.created_end} onChange={e => setFilters({...filters, created_end: e.target.value})} />
            </div>
          </div>
          <div className="space-y-3">
        ...
        <div className="flex gap-3 pt-4 border-t border-slate-50">
          ...
          <button onClick={downloadExcel} className="bg-green-600 text-white px-8 py-3.5 rounded-2xl font-black text-xs hover:bg-green-700 transition-all shadow-lg shadow-green-100">📊 엑셀 다운로드</button>
        </div>
      </div>
                    ...
                    <tr className="bg-slate-50/50">
                      <td colSpan={5} className="p-8 animate-in slide-in-from-top-2">
                        <div className="bg-white border-2 border-slate-100 rounded-[2.5rem] p-8 shadow-sm">
                          <div className="bg-blue-50 p-8 rounded-[2.5rem] mb-8 border border-blue-100 font-bold">
                            <div className="flex justify-between items-start mb-6">
                              <div>
                                <p className="text-[10px] font-black text-blue-400 uppercase mb-2">Order Summary</p>
                                <h3 className="text-2xl font-black text-slate-800 tracking-tight">
                                  {item.loading_place} <span className="text-blue-600 mx-1">→</span> {item.unloading_place} {item.unloading_place_2 && `→ ${item.unloading_place_2}`}
                                </h3>
                              </div>
                            ...
                            <div className="grid grid-cols-3 gap-8 border-t border-blue-100 pt-6">
                              <div className="space-y-2 text-xs">
                                <p className="font-black text-slate-400 uppercase">Loading</p>
                                <p className="text-sm">{item.loading_place} ({item.loading_time})</p>
                                <p className="text-slate-500">{item.loading_address}</p>
                                <p className="text-blue-600">담당: {item.loading_manager} ({item.loading_phone})</p>
                              </div>
                              <div className="space-y-2 text-xs border-l pl-8">
                                <p className="font-black text-slate-400 uppercase">Unloading 1</p>
                                <p className="text-sm">{item.unloading_place} ({item.product_name})</p>
                                <p className="text-slate-500">{item.unloading_address}</p>
                                <p className="text-slate-600">담당: {item.unloading_manager} ({item.unloading_phone})</p>
                              </div>
                              <div className="space-y-2 text-xs border-l pl-8">
                                <p className="font-black text-slate-400 uppercase">Unloading 2</p>
                                <p className="text-sm">{item.unloading_place_2 || "-"}</p>
                                <p className="text-slate-500">{item.unloading_place_2 ? item.unloading_address_2 : ""}</p>
                                <p className="text-slate-600">{item.unloading_place_2 ? `담당: ${item.unloading_manager_2} (${item.unloading_phone_2})` : ""}</p>
                              </div>
                            </div>
      ...
      {showOrderModal && (
        <div className="fixed inset-0 bg-[#1a1c2e]/60 backdrop-blur-md flex justify-end p-4 z-50 overflow-hidden">
          <div className="bg-white w-full max-w-2xl rounded-[3.5rem] shadow-2xl p-12 overflow-y-auto animate-in slide-in-from-right duration-300 relative">
            ...
            <h2 className="text-3xl font-black mb-8 uppercase text-slate-900 tracking-tighter">{selectedOrder ? '배차 수정' : '신규 배차'}</h2>
            <div className="space-y-6 font-black">
              ...
              <section className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <select onChange={e => { const s = staffs.find(x => x.name === e.target.value); setFormData(prev => ({...prev, loading_manager: e.target.value, loading_phone: s?.phone || ""})); }} className="p-5 bg-slate-50 rounded-2xl text-sm border-none shadow-inner text-blue-600 outline-none font-bold">
              ...
              {/* ✨ 하차지 섹션 1 & 2 */}
              {[1, 2].map(num => (
                <section key={num} className={`space-y-4 p-6 rounded-[2.5rem] shadow-inner font-bold ${num === 1 ? 'bg-slate-50' : 'bg-blue-50/50'}`}>
                  <div className="flex items-center gap-2 ml-2">
                    <div className={`w-1.5 h-4 rounded-full ${num === 1 ? 'bg-blue-600/40' : 'bg-purple-600/40'}`}></div>
                  ...
                  <input value={num === 1 ? formData.unloading_address : formData.unloading_address_2} placeholder={`하차지 ${num} 주소`} className="w-full p-5 bg-white rounded-2xl border-none text-sm shadow-sm font-bold" onChange={e => setFormData({...formData, [num === 1 ? 'unloading_address' : 'unloading_address_2']: e.target.value})} />
                  <input value={num === 1 ? formData.product_name : formData.product_name_2} placeholder={`📦 제품명 ${num}`} className="w-full p-5 bg-white rounded-2xl border-none text-sm shadow-sm" onChange={e => setFormData({...formData, [num === 1 ? 'product_name' : 'product_name_2']: e.target.value})} />
                </section>
              ))}
              ...
              <button onClick={handleOrderSubmit} className="w-full mt-10 p-6 bg-blue-600 text-white rounded-[2.5rem] text-xl shadow-xl shadow-blue-100 hover:bg-blue-700 transition-all uppercase tracking-widest">{selectedOrder ? '수정 완료 💾' : '신규 배차 저장 🚀'}</button>
            </div>
          </div>
        </div>
      )}
